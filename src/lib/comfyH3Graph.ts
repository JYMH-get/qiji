/**
 * ComfyUI H3 工作流图构建器（纯函数零 IO，可单测）——**客户端直连版**。
 *
 * ⚠ 与服务端 server/src/translators/comfyGraph.ts 是**同一骨架的两份拷贝**：
 *   服务端那份服务「奇迹云」渠道（管理端转发），本份服务「ComfyUI 直连」第三方本地渠道
 *   （comfyuiAdapter：客户端直连用户自己绑定的 ComfyUI 实例，不经管理端）。
 *   **改骨架/loader 模板/枚举映射时两处必须同步改**（跑同一个 jianyi933 工作流）。
 *
 ...
 *   删掉样例素材节点、剥掉 136 节点的 ref_* 键后替换 H3_SKELETON（节点 id 会漂，见下），并同步服务端那份。
 *
...
function nodeOfClass(graph: Record<string, any>, classType: string): any {
	...
	throw new Error(`工作流骨架缺少 ${classType} 节点——骨架被更换后未同步 comfyH3Graph.ts`);
}
...
function durationNode(graph: Record<string, any>): any {
	...
	throw new Error("工作流骨架缺少「视频时长」节点（PrimitiveFloat）——骨架被更换后未同步 comfyH3Graph.ts");
}
...
export function buildH3Graph(opts: H3GraphOpts): Record<string, any> {
	if (opts.workflow !== "jianyi933") {
		throw new Error(`未知工作流骨架「${opts.workflow}」：ComfyUI 直连当前仅支持 jianyi933（MiniMax H3）工作流`);
	}
