 ...
 *  1) **新选中**「分镜占位符」片段（placeholder+shotRef）→ 切「AI 工作台」；
 *     同一选中不重复切、取消选中不切（与 rtcPropsTabCore 同语义）；
...
export interface CenterSelSnapshot {
	...
	segId: string | null;
	/** 选中片段 kind（无选中=null；规则 3「placeholder→media」判定用） */
	segKind: string | null;
	/** 选中片段是否「占位符+shotRef」（AI 工作台的编辑目标；无 shotRef 的自由结果占位不算） */
	isShotPlaceholder: boolean;
...
export function centerTabAutoSwitch(prev: CenterSelSnapshot, next: CenterSelSnapshot): RtcCenterTab | null {
	...
	// 规则 1：新选中分镜占位符 → AI 工作台（同一选中不重复切；取消选中不切）
	if (next.isShotPlaceholder && next.segId !== null && next.segId !== prev.segId) return "workbench";
