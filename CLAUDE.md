...
- **用户可见五类**：角色 / **群像** / 场景 / 生物 / 物品（对应界面侧栏）。~~群像(G) 并入角色~~（2026-06-21 第28轮**改**：群像单设界面 `/frame-group`，存 `projectStore.crowds`，与画布节点分组 `groups` 无关；出图复用 `asset.character.image`）。剧本分析按编号前缀 `G→crowds` 自动分流。
...
- 2026-06-21 第38轮（OSS 配置改管理端可管，和模型一样）：新增 `server/src/store/settings.ts`(落盘 `data/settings.json`)——`getOssConfig()`=管理端值 ?? `.env`(config.oss) 回退、`setOssConfig()` 落盘+`ossConfigVersion()` 触发 S3 客户端重建。`oss.ts` 改读 settings(不再直读 config)，加 `ossSelfTest()`(上传临时对象→公网读→删，自检)。admin 加 `GET/PUT /admin-api/settings/oss`(secret 脱敏返回、留空不改) + `POST .../test`；控制台加「OSS存储」tab(端点/桶/AK/SK/区域/publicBase 表单 + 保存 + 测试连接 + 已配置徽标)。密钥只存服务端、用户端无感（同模型 key）。**实测**：tsx 跑 getOssConfig+ossSelfTest→ok、公网直链可读。`.env` 降级为可选 seed（可清空改用 /admin 管理）。server tsc 过。
- 2026-06-21 第37轮（OSS 资产落盘·Phase 1 = 服务端先行）：接雨云 rains3（S3 兼容，公有读）。新增 `@aws-sdk/client-s3` + `server/src/store/oss.ts`(ossPut/ossPublicUrl/isOssConfigured)；config 加 `oss`(endpoint/bucket/key/region/publicBase=`https://<bucket>.<host>`)。重写 `store/assets.ts`：**类型前缀 id**(C/A/G/M/S/P/video/audio，`nextAssetId(prefix)` 分前缀单调) + 元数据落盘 `data/assets.json`(seqs+records，重启不丢不复用) + 配 OSS 则上传得公网直链(`assets/<id>.<ext>`)否则内存兜底；`AssetRecord` 加 `name/url/ossKey`，去 `data`(字节走 OSS 或 `getAssetBytes` 内存)。translators：`assetPrefixFor(req)`(可 `params.idPrefix` 覆盖)+`assetNameOf`，createAsset 改 async 传 prefix+name；图/stub 落 `asset.url`(OSS)。gemini 底图改 `getAssetBytes` 或 OSS url 拉取。routes：`/raw` 有 OSS url 则 302 重定向、否则内存兜底；上传 `/v1/assets` 收 `prefix/name`。客户端 AssetWorkbench 传 `idPrefix`(按 cat：C/G/S/M/P)+`assetName`。**实测**：node 直连真桶 PUT→公网 GET 200→删除，OSS OK。两端 tsc+build 过。密钥仅入 `server/.env`(gitignore)。**Phase 2 待做**：客户端本地副本(assets/ 目录)+用户上传直传 OSS+预签名+项目按 v3 多文件拆分+迁移本人旧数据+视频也落 OSS。
...
- 2026-06-21 第29轮（资产生成界面 UI 重制·角色页先行）：5 区布局重制（1 导航/2 资产列表/3 造型分体/4 提示词/5 图片展示）。新增共享组件 `src/components/AssetWorkbench.tsx`（区2-5；区3 造型分体卡列表=基础形象为第1张+变体，区4 提示词框+设计理念+音色+生成数量+生成形象/提示词优化，区5 主图+历史缩略图点选主图供绑定）。数据模型扩展：asset 与 `AssetVariantLite` 加 `images[]`(历史)+`image`(主图)，变体加 `prompt`；projectStore 加 `AssetCat` 类型 + 通用动作 `updateAsset/addAssetVariant/updateAssetVariant/addAssetImage/setAssetMainImage`。`Frame16285`(角色) 改为薄壳调用 AssetWorkbench。区2 一键生成=全资产基础形象；区3 一键生成=当前资产全造型；提示词优化=复用 `script.analyze` 文本模型润色回填。**本轮仅角色页**（用户定：确认后再推广场景/群像/生物/物品）；**音色选择暂为本地态**(未持久化/未接 TTS)。tsc+build 过。
- 2026-06-21 第28轮（新增群像界面 + 关联剧本分析）：推翻"群像并入角色"，群像(G 编号)单设界面。新增 `projectStore.crowds`(独立于画布节点分组 `groups`)+`updateCrowdImage`，贯穿 setAnalysisResult/save/load/new/import + `projectFile.QijiProject.crowds`。复制角色页为 `src/views/FrameGroup.tsx`(读 crowds，标题/文案改群像，出图复用 `asset.character.image`，造型预设→状态变体)，注册路由 `/frame-group` + EditorSidebar 加「群像」tab(角色后)。`parseAssetExtraction` 加 crowds 桶、`G→crowds` 分流(C/A 仍→角色)，Frame1693 落库传 crowds + 加「群像」统计卡。实测：用户剧本→角色7/群像1(赵家军)/场景5/物品4/生物0；tsc+build 过。
